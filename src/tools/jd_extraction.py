...
async def parse_jd_text_to_json(jd_text):
    ...
    prompt = f'''Give me a JSON from the provided job description, that should have the job_role as its key with a value having the actual job role name then a - followed by the given details of technical knowledge and \
    ...
    company_background as its other key having its value as the actual company name then a - followed by the provided background details, if no company name is provided but background details are then just writing those details without the company name and also if the background details are missing but the company name is present then again only write the company name there without any background details, also if none of background details or company name are present then just write company background is not provided in this key.
