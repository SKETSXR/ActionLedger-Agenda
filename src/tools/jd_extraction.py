...
# Module-level logger (used only for diagnostics; does not change return values)
...
async def parse_jd_text_to_json(jd_text: str) -> str:
    ...
    Returns the model's content (as a string) with any surrounding markdown code
    fences removed. On any exception, returns the sentinel string:
      "JD not contain any text"

    Functionality is intentionally unchanged:
      - Uses the configured `llm_jd` to invoke with a single HumanMessage.
      - Strips optional ``` or ```json fences from the model output.
      - Returns the raw JSON string (no client-side parsing).
      - Broad except: returns the same sentinel value as the original code.
    """
    llm = llm_jd
    ...
    try:
        ...
        # Preserve original behavior while providing diagnostic breadcrumbs in logs
