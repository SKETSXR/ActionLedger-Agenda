...
# new   # 3) If any MUST skill is <missing> or <repeated>, revise the topics until each MUST skill appears once and none are skipped.  
# new 2
# - <Pre-output verification (required)>:  
#    1) Compute MUST_SKILLS set from the annotated skill tree.  
#    2) Compute the union of all skills you placed in the three `focus_area` lists.  
#    4) If any MUST skills are still left out in any of the topics then ensure it is placed in the General Skill Assessment related topic, so that none of the MUST skills are left out 
...
#    - None of the MUST skills should be skipped or repeated.  
#    - If it is not possible to naturally place all MUST skills in project or case study topics, then include <all remaining MUST skills in the General Skill Assessment related topic>. This ensures that collectively the three topics always cover 100 percent of <MUST_SKILLS>.  
#    - After writing and covering all the MUST skills, you may optionally add `"high"` priority leaf skills (never domains) where they fit, preferably if evidence exists in the summary/projects.  
