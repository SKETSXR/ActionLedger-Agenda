...
- `total_question_threshold`: A threshold number of questions only for Deep dive/QA block questions but it should follow a constraint that in each Deep dive node total_question_threshold should be atleast 2, but the sum of total_question_threshold in each deep dive node + 1 + 1 in a topic should be equal to total number of questions in a topic as provided. Also for non Deep Dive / QA Blocks total_question_threshold shall be null. 
...
- Sum of total_question_threshold of each deep dive node of a topic should be same as the total number of questions for a given topic as provided - 2 : so there will be 1 opening node, 1 direct node always but the number of deep dive nodes can vary although maintaining the constraints.
