...
# Your task is to convert a set of <a given input summary of discussion walkthroughs for 3 topics> into <nodes>.  
...
2) Node budget & composition
   - Let T = total_no_questions_context (integer).
   - Output exactly T nodes (not T±1; exactly T).
   - Composition:
     • Node #1: Direct, graded=false  (the opener)
     • Nodes #2..#(T-1): Direct, graded=true
     • Node #T: Deep Dive, graded=true  (terminal)
   - Exactly ONE Deep Dive (the last node). None before the last.
