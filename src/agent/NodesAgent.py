...
from src.model_handling import llm_n as _llm_client
...
class NodesAgentConfig:
    ...
    log_level: int = getattr(
        logging, os.getenv("NODES_AGENT_LOG_LEVEL", "INFO").upper(), logging.INFO
    )
    ...
    llm_backoff_base_s: float = float(
        os.getenv("NODES_AGENT_LLM_RETRY_BACKOFF_SECONDS", "2.5")
    )
    ...
    tool_backoff_base_s: float = float(
        os.getenv("NODES_AGENT_TOOL_RETRY_BACKOFF_SECONDS", "1.5")
    )
    tool_max_workers: int = int(os.getenv("NODES_AGENT_TOOL_MAX_WORKERS", "8"))

    tool_log_payload: str = (
        os.getenv("NODES_AGENT_TOOL_LOG_PAYLOAD", "off").strip().lower()
    )
...
def _attach_thread_file_handler(thread_id: str) -> None:
    ...
    No-op if per-thread logging is disabled, thread_id is empty, or a handler
    for this thread is already attached.
    """
    ...
    # logs/<thread_id>/<AGENT_NAME>.log  (uses CFG.log_file)
...
def _summarize_nodes_result(payload: Any) -> str:
    ...
    try:
        ...
        topics = (
            data.get("topics_with_nodes")
            if isinstance(data, dict)
            else getattr(payload, "topics_with_nodes", None)
        )
        ...
        if isinstance(topics, list):
            ...
            for t in topics[:6]:
                ...
                if isinstance(td, dict):
                    ...
                    nm = (
                        getattr(td, "topic", None)
                        or getattr(td, "name", None)
                        or getattr(td, "title", None)
                        or getattr(td, "label", None)
                    )
                    nodes = getattr(td, "nodes", None)
                names.append(
                    (nm or "Unknown").strip() if isinstance(nm, str) else "Unknown"
                )
...
class NodesGenerationAgent:
    ...
    def _to_json_one(x: Any) -> str:
        return json.dumps(
            NodesGenerationAgent._to_primitive(copy.deepcopy(x)), ensure_ascii=False
        )
    ...
    async def nodes_generator(state: AgentInternalState) -> AgentInternalState:
        ...
        if after != snapshot:
            raise RuntimeError(
                "discussion_summary_per_topic mutated during node generation"
            )

        state.nodes = NodesSchema(
            topics_with_nodes=[
                t.model_dump() if hasattr(t, "model_dump") else t
                for t in topic_with_nodes_list
            ]
        )
    ...
    async def should_regenerate(state: AgentInternalState) -> bool:
        ...
        try:
            NodesSchema.model_validate(
                state.nodes.model_dump()
                if hasattr(state.nodes, "model_dump")
                else state.nodes
            )
            ...
            _log_retry(
                f"[NodesGen][ValidationError] Container NodesSchema invalid\n {ve}",
                _nodes_retry_counter,
            )
            _nodes_retry_counter += 1
            return True
        ...
        try:
            ...
            _log_retry(
                f"[NodesGen][ValidationError] Could not read topics_with_nodes: {e}",
                _nodes_retry_counter,
            )
            _nodes_retry_counter += 1
            return True
