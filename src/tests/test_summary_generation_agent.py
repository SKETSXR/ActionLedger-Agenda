...
async def test_summary_generator_populates_state(monkeypatch, inp, summary):
    ...
    assert (
        state.generated_summary.job_requirements.about_company_or_product
        == inp.job_description.company_background
    )
    assert (
        "Node" in state.generated_summary.job_requirements.company_expectations_tech
        or "Postgres"
        in state.generated_summary.job_requirements.company_expectations_tech
    )
