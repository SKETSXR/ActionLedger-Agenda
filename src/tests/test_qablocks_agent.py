...
async def test_qablocks_generator_and_policy(monkeypatch, inp, summary, topics, dspt, nodes):
    ...
    def _mk_block(i):
        ...
        items = [
            QAItem(qa_id=f"QA{i}-{k}", q_type="New Question", q_difficulty="Medium",
                   counter_type=None, example_questions=["a","b","c","d","e"])
            for k in range(1,6)
        ] + [
            QAItem(qa_id=f"QA{i}-6", q_type="Counter Question", q_difficulty="Medium",
                   counter_type="Twist", example_questions=["a","b","c","d","e"]),
            QAItem(qa_id=f"QA{i}-7", q_type="Counter Question", q_difficulty="Hard",
                   counter_type="Interrogatory", example_questions=["a","b","c","d","e"]),
        ]
        return QABlock(block_id=f"B{i}", guideline="follow guides", qa_items=items)

    topic1_set = QASet(topic=topics.interview_topics[0].topic, qa_blocks=[_mk_block(1), _mk_block(2)])
    topic2_set = QASet(topic=topics.interview_topics[1].topic, qa_blocks=[_mk_block(3)])

    # Stub inner graph to emit QASetsSchema for each ainvoke in sequence
    class QAStub:
    ...
    stub_payloads = [
        QASetsSchema(qa_sets=[topic1_set]),
        QASetsSchema(qa_sets=[topic2_set]),
    ]
    monkeypatch.setattr(QA, "_qa_inner_graph", QAStub(stub_payloads))
